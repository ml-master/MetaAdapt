...
def get_dataset(args, mode, dataset, path):
    assert dataset.lower() in ['constraint', 'coaid', 'fakecovid', 'antivax', 'liar', 'fever',
        'fakenews', 'gettingreal', 'pheme', 'health', 'gossipcop', 'gossipcop_style_based','gossipcop_integration_based',
        'gossipcop_story_based','gossipcop_content_based','politifact', 'multisource']
    ...
    elif dataset.lower() == 'gossipcop_style_train':
        return GossipCopStyleTrainDataset(args, mode, path)   
    elif dataset.lower() == 'gossipcop_style_test':
        return GossipCopStyleTestDataset(args, mode, path)   
    elif dataset.lower() == 'gossipcop_integration_based':
        return GossipCopIntegrationBasedDataset(args,mode,path)
    elif dataset.lower() == 'gossipcop_story_based':
        return GossipCopStoryDataset(args,mode,path)
    elif dataset.lower() == 'gossipcop_content_based':
        return GossipCopcontentDataset(args,mode,path)
...
class GossipCopDataset(AbstractDataset):
    ...
    def load_dataset(self):
        data = []
        labels = []
        real_paths = os.path.join(self.path,  'real.json')
        fake_paths = os.path.join(self.path,  'fake.json')
        
        with open(real_paths, 'r', encoding='utf-8') as file:
            real_data = json.load(file)
            for key,value in real_data.items():
                data.append(value.get('text'))
                labels.append(1)

        with open(fake_paths, 'r', encoding='utf-8') as file:
...
class GossipCopcontentDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'gossipcop_content_based'
    ...
    def load_dataset(self):
        ...
        with open(fake_paths, 'r', encoding='utf-8') as file:
            fake_data = json.load(file)
            for key,value in fake_data.items():
                data.append(value.get('generated_text_glm4'))
                labels.append(0)
...
class GossipCopStoryDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'gossipcop_story_based'
    ...
    def load_dataset(self):
        ...
        with open(fake_paths, 'r', encoding='utf-8') as file:
            fake_data = json.load(file)
            for key,value in fake_data.items():
                data.append(value.get('generated_text'))
                labels.append(0)
...
class GossipCopStyleTrainDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'gossipcop_style_train'
    
    def load_dataset(self):
        data = []
        labels = []
        real_paths = os.path.join(self.path,  'real_train.json')
        fake_paths = os.path.join(self.path,  'fake_train.json')
        
        with open(real_paths, 'r', encoding='utf-8') as file:
            real_data = json.load(file)
            for key,value in real_data.items():
                data.append(value.get('generated_text_t015'))
                labels.append(1)
...
class GossipCopStyleTestDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'gossipcop_style_test'
    
    def load_dataset(self):
        data = []
        labels = []
        real_paths = os.path.join(self.path,  'real_test.json')
        fake_paths = os.path.join(self.path,  'fake_test.json')
...
class GossipCopIntegrationBasedDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'gossipcop_integration_based'
    ...
    def load_dataset(self):
        ...
        with open(real_paths, 'r', encoding='utf-8') as file:
            real_data = json.load(file)
            for key,value in real_data.items():
                data.append(value.get('generated_text_t01'))
                labels.append(1)
